...
import type { Folga, Bloqueio } from '@/types'

function formatData(iso: string) {
  return new Date(iso + 'T12:00:00').toLocaleDateString('pt-BR', { weekday: 'short', day: '2-digit', month: '2-digit', year: '2-digit' })
}
...
export default function BarbeiroFolgas() {
  ...
  const { data: folgas,   isLoading: lfF } = useQuery({
    queryKey: ['barbeiro-folgas'],
    queryFn:  () => api.get<Folga[]>('/barbeiros/folgas').then(r => r.data),
  })
  const { data: bloqueios, isLoading: lfB } = useQuery({
    queryKey: ['barbeiro-bloqueios'],
    queryFn:  () => api.get<Bloqueio[]>('/barbeiros/bloqueios').then(r => r.data),
  })
  ...
  const { mutate: criarFolga } = useMutation({
    mutationFn: (d: FormFolga) => api.post('/barbeiros/folgas', d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['barbeiro-folgas'] }); success('Folga registrada.'); setModalFolga(false) },
    onError:   () => error('Erro', 'Não foi possível registrar a folga.'),
  })

  const { mutate: deletarFolga } = useMutation({
    mutationFn: (id: string) => api.delete(`/barbeiros/folgas/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['barbeiro-folgas'] }); success('Folga removida.') },
    onError:   () => error('Erro', 'Não foi possível remover.'),
  })

  const { mutate: criarBloqueio } = useMutation({
    mutationFn: (d: FormBloqueio) => api.post('/barbeiros/bloqueios', d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['barbeiro-bloqueios'] }); success('Bloqueio criado.'); setModalBloqueio(false) },
    onError:   () => error('Erro', 'Não foi possível criar o bloqueio.'),
  })

  const { mutate: deletarBloqueio } = useMutation({
    mutationFn: (id: string) => api.delete(`/barbeiros/bloqueios/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['barbeiro-bloqueios'] }); success('Bloqueio removido.') },
    onError:   () => error('Erro', 'Não foi possível remover.'),
  })
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto flex flex-col gap-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Button variant="outline" size="sm" leftIcon={<Plus className="w-3.5 h-3.5" />}
            onClick={() => { resetF({}); setModalFolga(true) }}>
            Adicionar
          </Button>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Button variant="outline" size="sm" leftIcon={<Plus className="w-3.5 h-3.5" />}
            onClick={() => { resetB({}); setModalBloqueio(true) }}>
            Adicionar
          </Button>
        </div>
      ...
      <Modal isOpen={modalFolga} onClose={() => setModalFolga(false)} title="Registrar folga" size="sm"
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button variant="primary" size="md" loading={sF} form="form-folga" type="submit">Salvar</Button>
          </div>
        }
      >
      ...
      <Modal isOpen={modalBloqueio} onClose={() => setModalBloqueio(false)} title="Bloquear horário" size="sm"
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button variant="primary" size="md" loading={sB} form="form-bloqueio" type="submit">Salvar</Button>
          </div>
        }
      >
